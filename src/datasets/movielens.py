...
def reindex_df_ml25m(movies, ratings, tagging, genome_tagging, genome_tags):
    ...
    raw_genome_tid2genome_tid = {raw_genome_tid: genome_tid for raw_genome_tid, genome_tid in
                                 zip(unique_genome_tids, genome_tids)}
...
def generate_mlsmall_hete_graph(
    ...
    num_nodes = num_uids + num_iids + num_genres + num_years + num_directors + num_actors + num_writers + \
                num_tids
    num_node_types = 5 + sum([num_directors > 0, num_actors > 0, num_writers > 0])
    ...
    num_nodes_dict = {'uid': num_uids, 'iid': num_iids, 'genre': num_genres, 'year': num_years,
    ...
    print('1. Creating rating property edges...')
    ...
    print('2. Building edge type map...')
    ...
    print('3. Building the item occurrence map...')
...
class MovieLens(Dataset):
        ...
        if self.name == '25m':
            ...
            try:
                ...
                try:
                    ...
                    # in case no columns exist in df
                    pass
                # save dfs
                print('Saving processed csv...')
                save_df(tags, join(self.processed_dir, 'tags.csv'))
    ...
    def cf_negative_sampling(self):
        ...
        pos_edge_index_trans_np = self.edge_index_nps['user2item'].T  ##comment: uid --> iid, no rating
        num_interactions = pos_edge_index_trans_np.shape[0]
        if self.cf_loss_type == 'BCE':  # binary cross-entropy
        ...
        elif self.cf_loss_type == 'BPR':  # Bayesian Personalized Ranking
    ...
    def negative_sampling(self):
        ...
        self.train_data = torch.cat([cf_train_data[:self.train_data_length], kg_train_data[:self.train_data_length]],
                                    dim=1)
